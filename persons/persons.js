...
function findAuthorityData (trigger, searchterm, fid) {
    var viaf_url_suggest = 'https://www.viaf.org/viaf/AutoSuggest?query=';
    var cnt = 0;
    ...
    $.getJSON(viaf_url_suggest + encodeURIComponent(searchterm) + '&callback=?').done(function (result) {
        ...
        if (all_results != null) {
            // filter results by nametype of 'personal' to get only person authority data
            var person_results = all_results.filter(function (e) {
                return e.nametype == 'personal';
            });
            // filter results by nametype of 'personal' to get only person authority data
